...
class Twig
{
    ...
    private function _set_template_locations($theme)
    {
            ...
            if (! empty($module)) {
                foreach (Modules::$locations as $location => $offset) {
                    if (is_dir($location . $this->_module . '/views/'))
                        ...
                        $this->_template_locations[] = $location . $this->_module . '/views/';
                }
            }
        }
        ...
        // /themes/<theme name>
        $this->_template_locations[] = FCPATH . $this->_config['theme_dir'] . $theme;
    ...
    {
        $this->_twig->addFilter(new Twig_SimpleFilter('base_url', 'base_url'));
        $this->_twig->addFilter(new Twig_SimpleFilter('site_url', 'site_url'));
        $this->_twig->addFilter(new Twig_SimpleFilter('lang', 'lang'));
        $this->_twig->addFilter(new Twig_SimpleFilter('form_open', 'form_open'));
        $this->_twig->addFilter(new Twig_SimpleFilter('form_close', 'form_close'));
        $this->_twig->addFilter(new Twig_SimpleFilter('form_input', 'form_input'));
        $this->_twig->addFilter(new Twig_SimpleFilter('form_checkbox', 'form_checkbox'));
        $this->_twig->addFilter(new Twig_SimpleFilter('form_submit', 'form_submit'));
